...
local function checkAndPayBounty(killer, target)
    if not killer or not killer:isPlayer() then
        return false
    end

    local resultId = db.storeQuery(string.format("SELECT * FROM `bounties` WHERE `target` = %s AND `active` = 1 LIMIT 1",
        db.escapeString(target:getName():lower())))
    
    if resultId ~= false then
        local itemId = result.getNumber(resultId, "item_id")
        local itemCount = result.getNumber(resultId, "item_count")
        local placedBy = result.getString(resultId, "placed_by")
        result.free(resultId)
        
        -- Update bounty status
        db.query(string.format("UPDATE `bounties` SET `active` = 0 WHERE `target` = %s AND `active` = 1",
            db.escapeString(target:getName():lower())))
        
        -- Give the specific items
        killer:addItem(itemId, itemCount)
        
        -- Broadcast the message
        Game.broadcastMessage(string.format("[Bounty Hunter] %s has claimed the bounty by killing %s!",
    killer:getName(), target:getName()), MESSAGE_STATUS_WARNING)
        return true
    end
    return false
end
...
function onDeath(player, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
    ...
    -- Check for bounty first
    if killer and killer:isPlayer() then
        checkAndPayBounty(killer, player)
    end
