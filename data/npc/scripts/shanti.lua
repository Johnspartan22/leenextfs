...
local minX, minY, maxX, maxY = 155, 115, 156, 116
...
function onThink()
    ...
    if npcPos.x < minX or npcPos.x > maxX or npcPos.y < minY or npcPos.y > maxY then
        -- Move the NPC back to a safe position within the boundary
        doTeleportThing(getNpcCid(), {x = (minX + maxX) / 2, y = (minY + maxY) / 2, z = npcPos.z})
    end
    npcHandler:onThink()
end
