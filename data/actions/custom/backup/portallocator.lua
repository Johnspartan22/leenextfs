...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if foundPortal then
        ...
        if foundPos.z > playerPos.z then
            message = string.format("A rank %s portal is on a higher level", portalRank)
        elseif foundPos.z < playerPos.z then
            message = string.format("A rank %s portal is on a lower level", portalRank)
        else
            if distance == "beside" then
                message = string.format("A rank %s portal is next to you", portalRank)
            else
                message = string.format("A rank %s portal is %s to the %s", portalRank, distance, direction)
            end
        end
