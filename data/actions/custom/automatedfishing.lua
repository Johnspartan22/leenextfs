...
-- Use a global table to track player cooldowns - OTX compatible
if not FISHING_COOLDOWNS then
    FISHING_COOLDOWNS = {}
end
...
function onUse(cid, item, frompos, item2, topos)
    -- Get player GUID instead of using cid directly
    local playerGUID = getPlayerGUID(cid)
    local currentTime = os.time()

    -- Check if the player has triggered fishing in the past 60 seconds
    if FISHING_COOLDOWNS[playerGUID] and (currentTime - FISHING_COOLDOWNS[playerGUID]) < COOLDOWN_TIME then
        local remainingTime = COOLDOWN_TIME - (currentTime - FISHING_COOLDOWNS[playerGUID])
    ...
    if isInArray(waterIds, item2.itemid) == TRUE then
        if item2.itemid ~= 493 then  -- Exclude wells
            ...
            if useWorms == FALSE or (useWorms == TRUE and doPlayerRemoveItem(cid, ITEM_WORM, 1)) then
                -- Set the player's fishing cooldown using GUID
                FISHING_COOLDOWNS[playerGUID] = currentTime
                ...
                createRippleEffect(cid, topos, startPosition, 60, 5, item.itemid)  -- Ripple effect every 5 seconds for 60 seconds
