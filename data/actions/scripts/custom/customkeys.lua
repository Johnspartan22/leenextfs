...
REWARDS_ROOM_ACTIVE_STORAGE = 93000  -- Add this constant
...
local function getRewardsRoomStorage(playerId)
    return REWARDS_ROOM_ACTIVE_STORAGE + playerId + 1
end
...
function useKeyOfPower(player, keyId)
    ...
    -- Check if player is in rewards room
    local rewardsRoomStorageKey = getRewardsRoomStorage(playerId)
    local isInRewardsRoom = player:getStorageValue(rewardsRoomStorageKey)
    
    if isInRewardsRoom >= 1 then
        player:sendTextMessage(MESSAGE_STATUS_WARNING, "You cannot use keys while in a rewards room!")
        return false
    end
    ...
    -- If no active dungeon or rewards room, proceed with normal key functionality
