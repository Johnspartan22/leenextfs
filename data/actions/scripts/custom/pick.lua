function onUse(cid, item, frompos, item2, topos)
    ...
    -- Define better pick required rocks
    local betterPickRequired = {
        [4330] = true, -- Silver Vein
        [4331] = true, -- Gold Vein
    ...
    -- Define vein types and their depleted versions
    local veinTypes = {
        [4328] = {depleted = 1359, ore = 4362, name = "iron"}, -- Iron vein -> depleted rock, gives iron ore
        [4329] = {depleted = 1359, ore = 4363, name = "coal"}  -- Coal vein -> depleted rock, gives coal
    }
    
    -- Check if rock requires better pick
    if betterPickRequired[item2.itemid] then
        doPlayerSendTextMessage(cid, 22, "You need a better pick to mine this rock.")
        return false
    end
    
    -- Check if target is a valid vein
    if not veinTypes[item2.itemid] then
        doPlayerSendCancel(cid, "You can only mine from iron or coal veins.")
        return false
    end
    
    -- Level requirement
    if Level < 20 then
        doPlayerSendCancel(cid, "You need level 20 to mine.")
        return false
    end
    ...
    if rand <= 150 then
        ...
        if rand <= 200 and rand > 150 and item.itemid == 2553 then -- 5% chance for Drukus when using basic pick (range 151-200)
            -- Spawn Drukus using correct function for Avesta 0.6.3
            ...
            if drukus then
                ...
                addEvent(function()
                    if isCreature(drukus) then
                        -- Give better pick (you can change itemid 2554 to whatever better pick you want)
                        ...
                        end, 3000) -- Despawn after 3 seconds
                    end
                end, 2000) -- Give pick and say second message after 2 seconds
            end
